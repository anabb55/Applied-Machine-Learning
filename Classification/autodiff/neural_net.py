...
class Neuron(Module):
    ...
    def __call__(self, x: List[Scalar]) -> Scalar:
        ...
        """
        out = sum(x_i * w_i for x_i, w_i in zip(x, self.w))
...
class MultiLayerPerceptron(Module):
    def __init__(self, num_inputs: int, num_hidden: List[int], num_outputs: int):
        ...
        layerInputs = num_hidden.copy()
        layerInputs.insert(0, num_inputs)

        layerOutputs = num_hidden.copy()
        layerOutputs.append(num_outputs)
        self.layers = []

        for i in range(len(layerInputs)):
            useRelu = (i < len(layerInputs) -1) # the output layer should not use relu
            self.layers.append(FeedForwardLayer(layerInputs[i], layerOutputs[i], use_relu=useRelu))
    ...
    def __call__(self, x: List[Scalar]) -> List[Scalar]:
        ...
        for l in self.layers:
            output = l(output) # allways returns the input -> the probelem is the call method of layers?
